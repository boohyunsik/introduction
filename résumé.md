...
사용자가 의상 사진을 업로드하면, 비슷한 옷 12가지를 추천해주는 간단한 애플리케이션 입니다. 이미지 분석에서는 *OpenCV*의 *Feature matching*을 이용했습니다. 
...
koa는 lexer-parser-compiler를 거쳐서 Virtual-Machine이 읽는 byte code로 변환되는데, 저는 그 중에서 **parser** 개발을 담당했습니다. parser 개발에는 *Pratt parser*라 불리는 방식을 사용했습니다. 이 방식은 lexer에 의해 나뉜 token들을 재귀적으로 분석하는 방식입니다. 
